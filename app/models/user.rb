...
#  validates_length_of :login, :within => 3..40, :if => :not_using_openid?
 ...
 # validates_format_of :login, :with => RE_LOGIN_OK, :message => MSG_LOGIN_BAD, :if => :not_using_openid?
  ...
  attr_accessible :name, :email, :password, :password_confirmation, :last_name, :street, :city, :state, :country, :zipcode, :timezone_id, :phone_id, :referral_id, :phone_number,
  				  :ref_is_referred, :ref_first_name, :ref_last_name, :ref_phone_number
